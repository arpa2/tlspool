...
bool pgp_initcursor_radix64 (pgpcursor_t crs, char *data, uint32_t len) {
	...
	crs->ofs = 0;
	crs->r64ofs = r64ofs;
	// Count the characters of base64; look for '-' and '=' endings
	...
	while (1) {
		...
		if (r64ofs + r64len >= len) {
			// Unterminated text form, return failure
			return 0;
		}
		// Test if the current character is a base64 character
		if (strchr ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", data [r64ofs]) != NULL) {
			r64len++;
			continue;
		}
...
bool pgp_enter (pgpcursor_t seq, uint8_t *tag, pgpcursor_t sub) {
	...
	if (here & 0xc0 != 0xc0) {
		// Not a proper new-packet tag (we deprecate old 2.6.x style)
		return 0;
	}
	*tag = (here & 0x3f);
	if (!pgp_getbyte (seq, &here)) {
